...
foreign import ccall runInWKWebView :: StablePtr (WKWebView -> IO ()) -> CString -> CChar -> StablePtr (CString -> IO ()) -> IO ()
...
run didRegisterForRemoteNotificationsWithDeviceToken f = do
    ...
    withCString progName $ \pn -> runInWKWebView handler pn 1 deviceTokenHandler
...
runFile url allowing cfg f = do
    ...
    let registerForRemoteNotifications = boolToCChar $ _appDelegateConfig_registerForRemoteNotifications cfg
